...
#ifndef FNL_IMPL
#    define FNL_IMPL
#    include "../lib/fast_noise_lite.h"
#endif

#define COORD_SCALE_X 1.0f
#define COORD_SCALE_Y 1.0f

#define SIMPLEX_ANIMATION_SPEED_MAGNITUDE 30.0f
...
static const color_point_f_t noise_gradient_maps[3][GRADIENT_MAP_SIZE] = {
    // Profile 0: Lava (Blue → Pink → Orange)
    {{0.0f, {0.043f, 0.0f, 1.0f}}, {0.155f, {0.043f, 0.0f, 1.0f}}, {0.607f, {1.0f, 0.0f, 0.95f}}, {0.782f, {1.0f, 0.270f, 0.0f}}, {1.0f, {1.0f, 0.270f, 0.0f}}},
    // Profile 1: Matrix (Cyan → Neon Green → Lime)
    {{0.0f, {0.0f, 0.83f, 1.0f}}, {0.155f, {0.0f, 0.83f, 1.0f}}, {0.607f, {0.0f, 1.0f, 0.53f}}, {0.782f, {0.8f, 1.0f, 0.0f}}, {1.0f, {0.8f, 1.0f, 0.0f}}},
    // Profile 2: Solar (Red → Orange → Yellow)
    {{0.0f, {1.0f, 0.0f, 0.0f}}, {0.155f, {1.0f, 0.0f, 0.0f}}, {0.607f, {1.0f, 0.6f, 0.0f}}, {0.782f, {1.0f, 0.93f, 0.0f}}, {1.0f, {1.0f, 0.93f, 0.0f}}},
};
...
static inline float_rgb_t interpolate_gradient_f(float val) {
    const color_point_f_t *grad = noise_gradient_maps[profile_get_current_index()];
    if (val <= grad[0].position) return grad[0].color;
    if (val >= grad[GRADIENT_MAP_SIZE - 1].position) return grad[GRADIENT_MAP_SIZE - 1].color;
    for (uint8_t i = 0; i < GRADIENT_MAP_SIZE - 1; i++) {
        if (val <= grad[i + 1].position) {
            const float_rgb_t c0 = grad[i].color;
            const float_rgb_t c1 = grad[i + 1].color;
            const float       p0 = grad[i].position;
            const float       p1 = grad[i + 1].position;
    ...
    return grad[GRADIENT_MAP_SIZE - 1].color;
}

static inline RGB color_float_to_rgb8(const float_rgb_t *frgb, uint8_t sat8, uint8_t val8) {
...
static bool SIMPLEX_NOISE(effect_params_t *params) {
    ...
    for (uint16_t i = led_min; i < led_max; i++) {
        ...
        FNLfloat x_fnl = (float)g_led_config.point[i].x * COORD_SCALE_X + current_x_scroll_input;
        FNLfloat y_fnl = (float)g_led_config.point[i].y * COORD_SCALE_Y + current_y_scroll_input;
